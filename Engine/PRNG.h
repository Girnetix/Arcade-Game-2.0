...
public:
	NumberGenerator()
	{
		...
		RandomInit();
		pTimer->SetTimer((int)Randomize(300000, 600000), [this]() {this->UpdateGenerator(); }, true, 1);	//созадть таймер в пределах от 5 до 10 минут, для обновления генератора
	}
	...
	_Ty Randomize()
	{
		...
		y = y % (maxRange - minRange + 1) + minRange;
...
private:
	...
	_Ty m_mt[N];
private:
	_Ty Randomize(_Ty minRange, _Ty maxRange)
	{
		_Ty y = (m_mt[m_index] & MASK_UPPER) | (m_mt[(m_index + 1) % N] & MASK_LOWER);
...
extern default_number_generator* pNumGenerator;
